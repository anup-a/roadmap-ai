...
export function validateLesson(lesson, { warmup = [] } = {}) {
  ...
  const words = blocks.flatMap(blockTexts).filter((t) => typeof t === 'string').reduce((n, t) => n + wordCount(t), 0);
  const errors = [
    ...checkHeader(lesson),
  ...
  return { ok: errors.length === 0, errors, stats: { words, blocks: blocks.length, sources: (lesson.sources ?? []).length } };
}
